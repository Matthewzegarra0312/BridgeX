...
import { ReportGenerator, type ReportPeriod, type ReportData } from './utils/report-generator.js';
import { MySQLConfigSchema, type MySQLConfig } from './mcp/mysql-types.js';
import { GmailConfigSchema, type GmailConfig } from './mcp/gmail-types.js';
...
export class ReportAgent {
  ...
  private mysqlServer: MySQLMCPServer;
  private gmailServer: GmailMCPServer;
  ...
  private setupRoutes(): void {
    ...
    this.app.use('/api/test', createConfigRouter());
  ...
  async generateReport(params: {
    ...
    try {
      ...
      let previousPeriodData: ReportData | undefined;
      ...
      const report = await ReportGenerator.generateReport(
        params.reportType || this.config.reports.defaultReportType,
        params.period,
        reportData,
        previousPeriodData
      );
   ...
   */
  async runScheduledReport(reportType: string, recipients: string[]): Promise<any> {
    const queries = ReportGenerator.getReportQueries();
  ...
  private async executeQuery(sql: string, params: Record<string, any>): Promise<any> {
    ...
    const result = await (mysqlServer as any).callTool('mysql_query', {
      sql,
      params,
    });
   ...
   */
  private async sendReportEmail(params: any): Promise<any> {
    // En implementación real, esto haría una llamada HTTP al Gmail MCP Server
    const gmailServer = this.gmailServer.getMCPServer();
    
    // Esta es una simulación - en el mundo real se haría vía HTTP
    const result = await (gmailServer as any).callTool('gmail_send_report', params);
   ...
   */
  private calculatePreviousPeriod(period: ReportPeriod): ReportPeriod {
