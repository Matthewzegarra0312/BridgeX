...
export class GmailMCPServer {
  ...
  async authenticate(): Promise<void> {
    try {
      ...
      // Verificar autenticación obteniendo el perfil SOLO si no es una service account
      // Las service accounts no tienen perfil de usuario
      if (!this.config.impersonateEmail.includes('.iam.gserviceaccount.com')) {
      ...
      throw new GmailAuthError(
        `Cannot authenticate with Gmail API: ${error instanceof Error ? error.message : 'Unknown error'}`,
